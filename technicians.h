...
public:
    ...
    void print_route();
    void end_route();
    Technician swap_segment_and_point();
    Technician insert_segment_after_point();
private:
    vector<int> route;
...
Technician Technician::swap_segment_and_point(){
...
Technician Technician::insert_segment_after_point(){
    int route_len = (this -> route).size();
    if (route_len < 5) return *this;
    int segment[] = {rand_range(1, route_len - 2), -1}, point = rand_range(1, route_len - 1);
    segment[1] = segment[0] + 1;
